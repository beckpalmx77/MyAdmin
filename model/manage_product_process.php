...
if ($_POST["action"] === 'ADD') {
    ...
    if ($_POST["email"] !== '') {
        ...
        $sql_find = "SELECT * FROM ims_product WHERE email = '" . $email . "'";

        $nRows = $dbh->query($sql_find)->fetchColumn();
        if ($nRows > 0) {
            echo 2;
        } else {
            $sql = "INSERT INTO ims_product(user_id,email,password,first_name,last_name,account_type,picture,status)
...
if ($_POST["action"] === 'UPDATE') {
    ...
    if ($_POST["email"] != '') {
        ...
        $nRows = $dbh->query($sql_find)->fetchColumn();
        if ($nRows > 0) {
            $sql_update = "UPDATE ims_product SET first_name=:first_name,last_name=:last_name,status=:status,account_type=:account_type
            ,picture=:picture
            WHERE id = :id";
...
if ($_POST["action"] === 'CHG') {
    try {
        ...
        $id = $_POST["login_id"];
        $sql_update = "UPDATE ims_product SET password=:password WHERE id = :id";
        $query = $dbh->prepare($sql_update);
        $query->bindParam(':password', $password, PDO::PARAM_STR);
        $query->bindParam(':id', $id, PDO::PARAM_STR);
        $query->execute();
