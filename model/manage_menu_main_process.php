...
if ($_POST["action"] === 'ADD') {
    if ($_POST["label"] !== '') {
        $main_menu_id = "M" . sprintf('%03s', LAST_ID($dbh, "menu_main", 'id'));
        $label = $_POST["label"];
        $link = $_POST["link"];
        $icon = $_POST["icon"];
...
## Total number of records without filtering
    $stmt = $dbh->prepare("SELECT COUNT(*) AS allcount FROM menu_main ");
    $stmt->execute();
    $records = $stmt->fetch();
    $totalRecords = $records['allcount'];

## Total number of records with filtering
    $stmt = $dbh->prepare("SELECT COUNT(*) AS allcount FROM menu_main WHERE 1 " . $searchQuery);
    $stmt->execute($searchArray);
    $records = $stmt->fetch();
    $totalRecordwithFilter = $records['allcount'];

## Fetch records
    $stmt = $dbh->prepare("SELECT * FROM menu_main WHERE 1 " . $searchQuery
